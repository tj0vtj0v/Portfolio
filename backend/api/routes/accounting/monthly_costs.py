...
from backend.core.database.dao.accounting.monthly_cost_dao import MonthlyCostDao
...
async def create_monthly_cost(
        monthly_cost: MonthlyCostSchema,
        transaction: DBTransaction,
        monthly_cost_dao: MonthlyCostDao = Depends()
) -> MonthlyCostSchema:
    """
    ...
    return MonthlyCostSchema.from_model(created)
...
async def get_monthly_costs(
    ...
    monthly_costs = [MonthlyCostSchema.from_model(monthly_cost) for monthly_cost in raw_data]
...
async def update_monthly_cost(
    ...
    except MonthlyCostDao.NotFoundException as e:
        raise HTTPException(status_code=e.status_code, detail=e.detail)
    except IntegrityError as e:
        raise HTTPException(status_code=HTTPStatus.CONFLICT, detail=e.detail)

    return MonthlyCostSchema.from_model(updated)
